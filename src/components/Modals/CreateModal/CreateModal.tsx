...
const CreateModal: React.FC<Props> = ({ id, titulo }) => {
  ...
  const [productTypeInfo, setProductTypeInfo] = useState<
    ProductTypeResponseDto[] | null
  >(null);
  const [serviceTypeInfo, setServiceTypeInfo] = useState<
    ServiceTypeResponseDto[] | null
  >(null);
      ...
      if (serviceNew) {
        ...
        if (serviceCreated) {
          ...
          if (employeeServicesNew) {
            const employeeServiceCreated = await employeeServiceAdapter.create(
              employeeServicesNew
            );
            console.log(
              "serviço employee criado" + JSON.stringify(employeeServiceCreated)
            );
    ...
    <S.ModalSection id={id}>
      <S.ModalContainer>
        ...
        <S.ModalBody>
          ...
          {titulo === "Produto" ? (
            <S.SelectType
              name="Selecione o tipo do produto"
          ...
          {titulo === "Serviço" && (
            <S.InputContainer>
              <S.SelectType
                name="Selecione o funcionário do serviço"
                onChange={(e) => handleChange(e, setFuncionario)}
              >
                <option value="" disabled selected>
                  Selecione o funcionário do serviço....
                </option>
                {/* {establishment?. && establishment.employees.map((data: { id: string | number, name: string }) => (
                                <option  value={data.id}> {data.name} </option>
                            ))} */}
              </S.SelectType>
            </S.InputContainer>
          ...
          {titulo === "Serviço" && (
            <S.SelectType
              name="Selecione o tipo do serviço"
          ...
          <S.ButtonsContainer>
            <S.ReturnButton onClick={handleClose}>Voltar</S.ReturnButton>
