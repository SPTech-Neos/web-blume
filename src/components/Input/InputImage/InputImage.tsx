...
const inputImage: React.FC<S.InputImageProps> = ({  }) => {
    ...
    return (
        <S.Row>
            <S.ColumnCenter> 
                <Subtitle weight={700} size="lg">Foto de Perfil</Subtitle>
                {previewImage != '' ? <S.ImagePreview src={previewImage} alt="Pré-Vizualização da Imagem" /> : <S.Round />}
                {/* {previewImage != '' && <S.Round />} */}
                
            </S.ColumnCenter>
            <S.Column>
                <S.InputImage onChange={(e: any) => handleChange(e)} id="input-image" />
                <Subtitle weight={600} size="md">{imageName == '' ? 'Nenhum arquivo encontrado' : imageName}</Subtitle>
