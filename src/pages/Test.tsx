    ...
    if (adapter) {
      ...
      switch (type) {
        case "Employee":
          ...
          switch (method) {
            case "login":
              ...
              break;
            case "create":
              const createResult = await employeeAdapter.create(
                params.createDto
              );
              setResult(createResult);
              break;
            case "update":
              const updateResult = await employeeAdapter.update(
                params.id,
                params.updateFields
              );
              ...
              break;
            case "getById":
              const getByIdResult = await employeeAdapter.getEmployeeById(
                params.id
              );
        ...
        case "Client":
          ...
          switch (method) {
            case "login":
              ...
              break;
            case "create":
              const createResult = await clientAdapter.register(
                params.createDto
              );
              setResult(createResult);
              break;
            case "update":
              const updateResult = await clientAdapter.update(
                params.id,
                params.updateFields,
                token
              );
              ...
              break;
            case "getById":
              const getByIdResult = await clientAdapter.getClientById(
                params.id,
                token
              );
              setResult(getByIdResult);
        ...
        case "Establishment":
          ...
          switch (method) {
            case "create":
              const createResult = await establishmentAdapter.register(
                params.createDto
              );
              setResult(createResult);
              break;
            case "update":
              const updateResult = await establishmentAdapter.update(
                params.id,
                params.updateFields
              );
              setResult(updateResult);
            ...
            case "getById":
              const getByIdResult =
                await establishmentAdapter.getEstablishmentById(params.id);
              setResult(getByIdResult);
              break;
            default:
              break;
        ...
        <optgroup label="Employee Adapter">
          ...
          <option
            value={JSON.stringify({
              type: "Employee",
              method: "update",
              params: { id: 1, updateFields: { name: "Updated Name" } },
            })}
          >
            Update
          </option>
        ...
        <optgroup label="Client Adapter">
          <option
            value={JSON.stringify({
              ...
              method: "login",
              params: {
                loginDto: { email: "client@example.com", password: "password" },
              },
            })}
          >
            Login
            ...
            value={JSON.stringify({
              ...
              params: {
                createDto: {
                  ...
                  local: local,
                  profilePic:
                    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQg3rBWVF3ujofB707ALZWHYPV2tY6NCml8jg&usqp=CAU",
