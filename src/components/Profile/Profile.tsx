...
const Profile: React.FC<S.ProfileProps> = ({
  profile,
  tipoperfil,
  username,
}) => {
  const [servicesInfo, setServicesInfo] = useState<ServiceResponseDto[] | null>(
    null
  );
  ...
  useEffect(() => {
    if (establishmentData) {
      const fetchEstablishmentData = async () => {
        const serviceData = await serviceAdapter.getServicesByEstablishmentId(
          Number(establishmentData.id)
        );
          ...
          <S.ContainerTags>
            {servicesInfo &&
              (Array.isArray(servicesInfo)
                ? servicesInfo
                    .slice(0, 2)
                    .map((service: ServiceResponseDto, index: number) => (
                      <S.TagsProfile key={index}>
